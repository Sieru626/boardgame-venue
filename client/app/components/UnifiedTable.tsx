...
export default function UnifiedTable({ roomId, userId, state, socket, drawCard, playCard, rollDice, deckEditorOpen }: Props) {
    ...
    React.useEffect(() => {
        ...
        if (state.chat.length > prevChatLength.current) {
            ...
            newMsgs.forEach((m: any) => {
                // Heuristic: "A が B を密告しました"
                if (m.message && m.message.includes('密告しました')) {
                    // Extract target name simply? Or find by ID if we had it. 
                    // For MVP, just shake ALL players to simulate tension, or try to match name.
                    // Let's shake everyone slightly for tension!
    ...
    const triggerShake = (playerId: string) => {
        setActiveEffects(prev => ({ ...prev, [playerId]: 'animate-shake-short' })); // Needs CSS
        setTimeout(() => {
    ...
    const handleDenounce = () => {
        ...
        // Simple MVP: Dropdown is better but prompt is faster to impl. 
        // Let's use a simple find logic or prompt for exact ID if debugging.
        // Better: Show a simple overlay to select.
        // For now: Prompt text -> Search player.
